...
class spatial_transformer(nn.Module):
    ...
    def _interpolate(self, im, x, y, out_size):
        ...
        base = self._repeat(torch.arange(0, num_batch).int() * dim1, out_height * out_width).cpu()
        ...
        Ia = torch.index_select(im_flat, dim=0, index=Variable(idx_a.long()).cpu())
        Ib = torch.index_select(im_flat, dim=0, index=Variable(idx_b.long()).cpu())
        Ic = torch.index_select(im_flat, dim=0, index=Variable(idx_c.long()).cpu())
        Id = torch.index_select(im_flat, dim=0, index=Variable(idx_d.long()).cpu())
        ...
        output = wa.cpu() * Ia.data.cpu() + wb.cpu() * Ib.data.cpu() + wc.cpu() * Ic.data.cpu() + wd.cpu() * Id.data.cpu()
        return output
