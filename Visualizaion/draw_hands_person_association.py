...
img_root = 'CLASP-DATA-102319/cam11exp2'
...
with open('data/hands_id_cam11exp2_102419.pkl','r') as f:
...
with Bar('Processing hands and people association visualization',max=len(persons['id'])) as bar:
    for frame_id in persons['id'].keys():
        img = cv2.imread(img_root+'/'+'frame'+str(frame_id).zfill(5)+'.jpg')
        ...
        cv2.imwrite('viz/cam11exp2_hands_id/{}.jpg'.format(frame_id),img)
        bar.next()
